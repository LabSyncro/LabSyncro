    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.getValue('borrowableQuantity') as number > 0 ?
          h('p', { class: 'relative text-green-500 pl-6 sm:pl-8' }, [h('span', { class: 'absolute left-0.5 top-[0.4] bg-green-500 rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center' }), [h(Icon, { name: 'i-heroicons-check', class: 'text-white text-sm font-bold' })], h('span', `${row.getValue('borrowableQuantity')} cái`)])
